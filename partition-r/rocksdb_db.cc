...
namespace {
  ...
  const std::string PROP_MAX_BG_FLUSHES_DEFAULT = "6";

  const std::string PROP_MAX_BG_COMPACTIONS = "rocksdb.max_background_compactions";
  const std::string PROP_MAX_BG_COMPACTIONS_DEFAULT = "1";
...
void RocksdbDB::GetOptions(const utils::Properties &props, rocksdb::Options *opt,
  ...
  if (options_file != "") {
    ...
    val =std::stoi(props.GetProperty(PROP_MAX_BG_FLUSHES, PROP_MAX_BG_FLUSHES_DEFAULT));
    if (val != 0) {
      opt->max_background_flushes = val;
    }
    val =std::stoi(props.GetProperty(PROP_MAX_BG_COMPACTIONS, PROP_MAX_BG_COMPACTIONS_DEFAULT));
    if (val != 0) {
...
const bool registered = DBFactory::RegisterDB("partition-r", NewRocksdbDB);
