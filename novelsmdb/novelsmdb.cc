...
namespace {
  ...
  const std::string PROP_TEST_PATH="leveldb.test_path";
  const std::string PROP_TEST_PATH_DEFAULT="/tmp/novelsm";

  const std::string PROP_PM_PATH="leveldb.pm_path";
  const std::string PROP_PM_PATH_DEFAULT="/mnt/pmemdir";

  const std::string PROP_LEVEL="leveldb.level";
  const std::string PROP_LEVEL_DEFAULT="2";

  const std::string PROP_PM_BUFFER_SIZE="leveldb.pm_buffer_size";
  const std::string PROP_PM_BUFFER_SIZE_DEFAULT="8589934592";

} // anonymous

namespace ycsbc {

leveldb::DB *NovelsmdbDB::db_ = nullptr;
int NovelsmdbDB::ref_cnt_ = 0;
std::mutex NovelsmdbDB::mu_;

void NovelsmdbDB::Init() {
  ...
  if (format == "single") {
    ...
    method_read_ = &NovelsmdbDB::ReadSingleEntry;
    method_scan_ = &NovelsmdbDB::ScanSingleEntry;
    method_update_ = &NovelsmdbDB::UpdateSingleEntry;
    method_insert_ = &NovelsmdbDB::InsertSingleEntry;
    method_delete_ = &NovelsmdbDB::DeleteSingleEntry;
  } else if (format == "row") {
    format_ = kRowMajor;
    method_read_ = &NovelsmdbDB::ReadCompKeyRM;
    method_scan_ = &NovelsmdbDB::ScanCompKeyRM;
    method_update_ = &NovelsmdbDB::InsertCompKey;
    method_insert_ = &NovelsmdbDB::InsertCompKey;
    method_delete_ = &NovelsmdbDB::DeleteCompKey;
  } else if (format == "column") {
    format_ = kColumnMajor;
    method_read_ = &NovelsmdbDB::ReadCompKeyCM;
    method_scan_ = &NovelsmdbDB::ScanCompKeyCM;
    method_update_ = &NovelsmdbDB::InsertCompKey;
    method_insert_ = &NovelsmdbDB::InsertCompKey;
    method_delete_ = &NovelsmdbDB::DeleteCompKey;
  } else {
  ...
  if (db_path == "") {
    ...
    const std::string &db_pm_path = props.GetProperty(PROP_PM_PATH, PROP_PM_PATH_DEFAULT);
    if (db_pm_path == "") {
        throw utils::Exception("LevelDB db path is missing");
    }
  ...
  if (props.GetProperty(PROP_DESTROY, PROP_DESTROY_DEFAULT) == "true") {
    s = leveldb::DestroyDB(db_path, db_pm_path,opt);
    if (!s.ok()) {
      throw utils::Exception(std::string("LevelDB DestroyDB: ") + s.ToString());
    }
  }
  s = leveldb::DB::Open(opt, db_path,db_pm_path, &db_);
  if (!s.ok()) {
...
void NovelsmdbDB::GetOptions(const utils::Properties &props, leveldb::Options *opt) {
  ...
  if (writer_buffer_size > 0) {
    ...
    size_t writer_pm_buffer_size = std::stol(props.GetProperty(PROP_PM_BUFFER_SIZE,
                                                               PROP_PM_BUFFER_SIZE_DEFAULT));
    if (writer_pm_buffer_size > 0) {
        opt->nvm_buffer_size = writer_pm_buffer_size;
    }
...
void NovelsmdbDB::SerializeRow(const std::vector<Field> &values, std::string *data) {
...
void NovelsmdbDB::DeserializeRowFilter(std::vector<Field> *values, const std::string &data,
...
void NovelsmdbDB::DeserializeRow(std::vector<Field> *values, const std::string &data) {
  const char *p = data.data();
  const char *lim = p + data.size();
  while (p != lim) {
    assert(p < lim);
...
std::string NovelsmdbDB::BuildCompKey(const std::string &key, const std::string &field_name) {
  switch (format_) {
    case kRowMajor:
...
std::string NovelsmdbDB::KeyFromCompKey(const std::string &comp_key) {
  size_t idx = comp_key.find(":");
  assert(idx != std::string::npos);
  return comp_key.substr(0, idx);
}

std::string NovelsmdbDB::FieldFromCompKey(const std::string &comp_key) {
  size_t idx = comp_key.find(":");
  assert(idx != std::string::npos);
  return comp_key.substr(idx + 1);
}

DB::Status NovelsmdbDB::ReadSingleEntry(const std::string &table, const std::string &key,
                                      const std::vector<std::string> *fields,
...
DB::Status NovelsmdbDB::ScanSingleEntry(const std::string &table, const std::string &key, int len,
                                      const std::vector<std::string> *fields,
...
DB::Status NovelsmdbDB::UpdateSingleEntry(const std::string &table, const std::string &key,
...
DB::Status NovelsmdbDB::InsertSingleEntry(const std::string &table, const std::string &key,
                                        std::vector<Field> &values) {
  std::string data;
...
DB::Status NovelsmdbDB::DeleteSingleEntry(const std::string &table, const std::string &key) {
  leveldb::WriteOptions wopt;
...
DB::Status NovelsmdbDB::ReadCompKeyRM(const std::string &table, const std::string &key,
                                    const std::vector<std::string> *fields,
                                    std::vector<Field> &result) {
  leveldb::Iterator *db_iter = db_->NewIterator(leveldb::ReadOptions());
  db_iter->Seek(key);
...
DB::Status NovelsmdbDB::ScanCompKeyRM(const std::string &table, const std::string &key, int len,
                                    const std::vector<std::string> *fields,
                                    std::vector<std::vector<Field>> &result) {
  leveldb::Iterator *db_iter = db_->NewIterator(leveldb::ReadOptions());
  db_iter->Seek(key);
...
DB::Status NovelsmdbDB::ReadCompKeyCM(const std::string &table, const std::string &key,
                                    const std::vector<std::string> *fields,
                                    std::vector<Field> &result) {
  return kNotImplemented;
}

DB::Status NovelsmdbDB::ScanCompKeyCM(const std::string &table, const std::string &key, int len,
                                    const std::vector<std::string> *fields,
                                    std::vector<std::vector<Field>> &result) {
  return kNotImplemented;
}

DB::Status NovelsmdbDB::InsertCompKey(const std::string &table, const std::string &key,
                                    std::vector<Field> &values) {
  leveldb::WriteOptions wopt;
...
DB::Status NovelsmdbDB::DeleteCompKey(const std::string &table, const std::string &key) {
  leveldb::WriteOptions wopt;
  leveldb::WriteBatch batch;
...
DB *NewNovelsmdbDB() {
  return new NovelsmdbDB;
}

const bool registered = DBFactory::RegisterDB("novelsm", NewNovelsmdbDB);
